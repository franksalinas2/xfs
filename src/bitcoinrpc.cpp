...
{
    return GetBoolArg("-testnet", false) ? 18762 : 8762;
}
...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "Stop FrankSalinasCoin server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "FrankSalinasCoin server stopping";
}
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: franksalinascoin-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
static string HTTPReply(int nStatus, const string& strMsg, bool keepalive)
{
    if (nStatus == HTTP_UNAUTHORIZED)
        return strprintf("HTTP/1.0 401 Authorization Required\r\n"
            "Date: %s\r\n"
            "Server: franksalinascoin-json-rpc/%s\r\n"
...
void ThreadRPCServer(void* parg)
{
    ...
    RenameThread("franksalinascoin-rpclist");
...
void ThreadRPCServer2(void* parg)
{
        ...
        (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"]))
    {
        ...
        string strWhatAmI = "To use franksalinascoind";
...
void ThreadRPCServer3(void* parg)
{
    ...
    RenameThread("franksalinascoin-rpchand");
